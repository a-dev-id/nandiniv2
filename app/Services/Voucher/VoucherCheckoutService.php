    ...
    public function createOrderAndPayment(array $data, $member = null): VoucherOrder
    {
        ...
        $order = DB::transaction(function () use ($data, $member, $cart, $accessToken): VoucherOrder {
            $order = VoucherOrder::query()->create([
                ...
                'metadata' => [
                    'guest_access_token_created' => true,
                    'pre_tax_total' => $cart['pre_tax_total'],
                    'cart_discount' => $cart['cart_discount'],
                    'cart_discount_percentage' => $cart['cart_discount_percentage'],
                    'service_charge' => $cart['service_charge'],
                    'service_charge_percentage' => $cart['service_charge_percentage'],
                    'tax' => $cart['tax'],
            ...
            foreach ($cart['lines'] as $line) {
                ...
                $order->items()->create([
                    ...
                    'voucher_snapshot' => [
                        ...
                        'tax' => $line['tax'],
                        'line_total_before_cart_discount' => $line['line_total'],
                        'cart_discount' => $line['cart_discount'],
                        'hotel_note' => $line['hotel_note'] ?? null,
                    ],
                ]);
            }
